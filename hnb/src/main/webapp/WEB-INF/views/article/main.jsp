...
$(function() {
     $.getJSON(context+ '/admin/member_list/1', function(data) {
           ...
           + '<TD WIDTH=320 ALIGN=CENTER>';
            if (data.startPage != 1) {
                pagination += '<a href="'+context'+/event/boardList/1">'
                + '<IMG SRC="${img}/btn_bf_block.gif">&nbsp;'
                + '</a>';
           }
            if ((data.startPage-data.groupSize) > 0) {
                pagination += '<a href="${context}/event/boardList/'+(data.startPage-data.groupSize)+ '">'
           ...
           + '<font style="color:red;font-size: 20px">${status.index}'
           + '</font>'
           + '</c:when>'
           + '<c:otherwise><a href="${context}/event/boardList/${status.index}">'
           + '<font>${status.index}'
           + '</font>'
           + '</a>'
           + '</c:otherwise>'
