...
<script type="text/javascript">
	...
	var LoginMember = {
			detail : function(url) {
				$.getJSON(url,
					function(data){
					var table = '<table id="tab_detail"><tr><td rowspan="10" id="td_profile"><img id="profile" src="${img}/'+data.profile+'" width="70%" height="80%"/></td>'
					+'<th id="item">항목</th><th>빈 칸</th></tr><tr><td>아이디</td><td>${user.id}</td></tr><tr><td>비밀번호</td><td>${user.password}'
					+'</td></tr><tr><td>이름</td><td>${user.name}</td></tr><tr><td>생일</td><td>${user.birth}</td></tr><tr>'
					+'<td>주소</td><td>${user.addr}</td></tr><tr><td>이메일</td><td>${user.email}</td></tr>'
					+'<tr><td>전화번호</td><td>${user.phone}</td></tr>'
					+'<tr><td>성별</td><td>${user.gender}</td></tr></tr><tr><td>등록일</td><td>${user.regdate}</td></tr><tr>'
			...
			updateForm : function() {
				$.getJSON(context+'/member/detail/${user.id}',
					...
					var table = '<table id="tab_detail"><tr><td rowspan="10" id="td_profile"><img id="profile" name="profile" src="${img}/'+data.profile+'" width="70%" height="80%"/></td>'
					...
					+'<tr><td>전화번호</td><td><input type="text" id="phone" name="phone" value ="'+data.phone+'"></td></tr>'
					+'<tr><td>성별</td><td>'+data.gender+'</td></tr><tr><td>등록일</td><td>'+data.regdate+'</td></tr><tr>'
